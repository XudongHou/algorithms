...
/**
 * 三向单词查找树
 * {@code TST}类表示键 - 值对的符号表，包含字符串键和通用值。它支持通常的put，get，contains，delete，size
 * 和is-empty方法。 它还提供基于字符的方法来查找符号表中的字符串，该符号表是给定前缀的最长前缀，查找符号表中以给定前缀开
 * 头的所有字符串，并查找符号表中符合 给定模式。 符号表实现关联数组抽象：当将值与已经在符号表中的键相关联时，惯例是用新值替换
 * 旧值。 与{@link java.util.Map}不同，此类使用的约定是值不能为{@code null} - 将与键相关联的值设置为
 * {@code null}等效于从符号表中删除键。 此实现使用三向搜索trie。
...
public class TST<Value> {
	...
	public static void main(String[] args) {
		...
		In in = new In(new File(".\\algs4-data\\shellsST.txt"));
		int i = 0;
		while (!in.isEmpty()) {
